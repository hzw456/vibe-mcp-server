use crate::models::{Claims, User, Config};

pub fn create_jwt_token(user: &User, config: &Config) -> Result<String, jsonwebtoken::errors::Error> {
...
pub fn decode_jwt_token(token: &str, config: &Config) -> Result<Claims, jsonwebtoken::errors::Error> {
    jsonwebtoken::decode(
        token,
